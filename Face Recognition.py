...
# The advantage of **Siamese Network** is that it allows a way to do this sort of verification task with very
# little user data, as it is quite unreasonable to train using thousands of images for each user.

# Here we will be using **FaceNet Model**.
# FaceNet learns a neural network that encodes a face image into a vector of 128 numbers.
# So by comparing two such vectors, we can then determine if two pictures are of the same person.
...
# Two encodings are compared and if the two encodings are similar then we say that the two images are of the same person
...
# ### Loading the Model
# The model outputs a vector of 128 numbers which represent encoding for the given input image.
# We will be using this encoding vector for comparing two images.
# #### Input
...
def load_emotion_model():
    FRmodel = load_model('models/emotional_model.h5')
    return FRmodel

# We will create a database of registered.
# For this we will use a simple dictionary and map each registered user with his/her face encoding.
...
# see if there is any similar encoding in the database or not.
# We define a threshold value to decide whether the two images are similar
# or not based on the similarity of their encodings.


def find_face(image_path, database, model, threshold=0.7):
...
def main():
    ...
    while ch == 'y' or ch == 'Y':
        ...
        if user_input == '1':
            ...
            detect_face_realtime(user_db, FRmodel, emotion_model, threshold=0.7)
